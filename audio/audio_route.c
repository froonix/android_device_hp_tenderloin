...
#if 0
		if (path->setting[i].ctl_vals == 3) {
			ALOGV("mixer_set: '%s' %ld,%ld,%ld\n",
					mixer_ctl_get_name(ctl),
					path->setting[i].value[0],
					path->setting[i].value[1],
					path->setting[i].value[2]
					);
		} else if (path->setting[i].ctl_vals == 2) {
			ALOGV("mixer_set: '%s' %ld,%ld\n",
					mixer_ctl_get_name(ctl),
					path->setting[i].value[0],
					path->setting[i].value[1]
					);
		} else if (path->setting[i].ctl_vals == 1) {
			ALOGV("mixer_set: '%s' %ld\n",
					mixer_ctl_get_name(ctl),
					path->setting[i].value[0]
					);
		} else {
